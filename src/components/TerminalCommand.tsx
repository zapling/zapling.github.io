...
export const TerminalCommand = ({remoteConnection, onTypingDone, children} : IProps) => {
    return (
        <>
            { getTerminalPrefix(remoteConnection) }
            { children !== undefined && <Typist cursor={''} typingDelay={10} onTypingDone={onTypingDone}>{ children} </Typist> }
        </>
    );
}
